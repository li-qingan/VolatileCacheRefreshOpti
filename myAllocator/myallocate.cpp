...
map<int, map<ADDRINT, map<ADDRINT, ADDRINT> > > g_graph;
// blocks to output 
map<int, list<CBlock *> > g_Blocks;
...
int Dump()
{
	...
	map<int, list<CBlock *> >::iterator I = g_Blocks.begin(), E = g_Blocks.end();
	for( ; I != E; ++ I)
	{
		int nFunc = I->first;
		os << endl << "###" << nFunc <<  endl;
...
int AdjacentTable(int nFunc, map<ADDRINT, set<ADDRINT> > &Atable)
{
...
int DoAllocate() 
{
    map<int, map<ADDRINT, map<ADDRINT, ADDRINT> > >::iterator i_p = g_graph.begin(), i_e = g_graph.end();
	for(; i_p != i_e; ++ i_p )
	{
		int nFunc = i_p->first;
